...
method syncWorkflows () {
	...
	my $email	=	$self->table()->db()->query($query);
...
method initialiseResource ($owner, $username, $package, $privacy, $opsdir, $installdir) {
	...
	my $not_defined = $self->table()->db()->notDefined($data, $required_fields);
...
method setWorkflowDir ($opsdir, $username) {
	...
	my $opsrepo = $self->conf()->getKey("core:OPSREPO");
...
method createProjectFiles ($username, $workflowdir) {
	...
	foreach my $projectdata ( @$projects ) {
		...
		$projectdata->{db}					=		$self->table()->db();
		$projectdata->{printlog}		=		$self->printlog();

		my $projectobject = Flow::Project->new($projectdata);
...
method loadProjectFromDatabase ($projectobject, $projectdata, $projectfile) {
	#### GET WORKFLOWS		
	my $workflows = $self->table()->getWorkflowsByProject($projectdata);
	...
	my $workflowobjects 	=	$projectobject->getWorkflowObjectsForProject($workflows, $username);
...
method loadProjectFiles ($username, $package, $installdir, $workflowdir) {
	$self->logDebug("BEFORE require Flow::Project");
	require Flow::Project;
	$self->logDebug("AFTER require Flow::Project");
	...
	foreach my $project ( @$projects ) {
		...
		$self->logDebug("Doing Flow::Project->new()");	
		...
		$projectobject->projectToDatabase($username, $projectobject);
		...
		foreach my $workflowobject ( @$workflowobjects ) {
			...
			my $found = $self->objectInArray($workflows, $workflowdata, $keys);
			$self->logDebug("Doing next because found", $found) if $found;
			next if $found;
			
			#### ADD WORKFLOW
			$workflowobject->workflowToDatabase($workflowdata);
			...
			foreach my $stageobject ( @$stageobjects ) {
				...
				#### ADD STAGE	
				$stageobject->stageToDatabase($username, $stageobject, $projectname, $workflowname, $workflownumber, $stagenumber);
				...
				foreach my $parameterobject ( @$parameterobjects ) {
					...
					#### ADD PARAMETER
					$stageobject->stageParameterToDatabase($username, $package, $installdir, $stageobject, $parameterobject, $projectname, $workflowname, $workflownumber, $stagenumber, $paramnumber);
				}
			}
		}
	}
...
method getAppsByPackage ($package) {
	...
	my $where = $self->table()->db()->where($package, $fields);
	my $query = qq{SELECT * FROM app $where};
	my $apps = $self->table()->db()->queryhasharray($query);
...
method _createAppFile ($app, $parameters, $username, $appdir) {
	...
	my $application = Flow::App->new($app);
...
method loadAppFiles ($username, $package, $installdir, $appdir, $format) {	
	require Flow::App;
	$self->logDebug("username", $username);
	$self->logDebug("package", $package);
	$self->logDebug("installdir", $installdir);
	...
	foreach my $typedir ( @$typedirs ) {
		...
		foreach my $appfile ( @$appfiles ) {
			...
			$self->logDebug("Loading inputfile into Flow::App", $inputfile);
			...
			$application->read();
			#$self->logDebug("application", $application->toString());
			...
			$self->logWarning("appToDatabase failed for application: $appname") and next if not defined $result or not $result;
			...
			foreach my $parameter (@$parameters) {
				my $success = $self->parameterToDatabase($owner, $username, $package, $installdir, $application, $parameter);
...
#### SOURCE FILES
method createSourceFiles ($username, $outputdir) {
	my $sources = $self->_getSources($username);
	...
	foreach my $source ( @$sources ) {
		...
		my $sourcedir = "$outputdir/sources/$sourcename";
		File::Path::mkpath($sourcedir) if not -d $sourcedir;
		$self->logError("Can't create sourcedir: $sourcedir") and exit if not -d $sourcedir;

		#### SET PROJECT FILE, REMOVE EXISTING FILE
		my $sourcefile = "$sourcedir/$sourcename.proj";
...
#### 	2. OTHERWISE, CREATE THE PRIVATE OPSREPO
	...
	my $opsrepo	= 	$self->conf()->getKey("core:OPSREPO");
	$opsrepo	=	$self->conf()->getKey("core:PRIVATEOPSREPO") if $privacy eq "private";
	$self->logDebug("opsrepo", $opsrepo);
...
method initOpsRepo ($login, $repository, $privacy) {
	...
	my $owner = $self->conf()->getKey("core:AGUAUSER");
...
method createPmFile ($file, $package) {
	my $content = qq{
use MooseX::Declare;
class $package extends Ops::Main {
}};
...
method setOpsRepo ($privacy) {
	...
	my $opsrepo;
	if ( $privacy eq "public" ) {
		$opsrepo = $self->opsrepo() || $self->conf()->getKey("core:OPSREPO");
		$self->logDebug("PUBLIC opsrepo", $opsrepo);
	}
	elsif ( $privacy eq "private" ) {
		$opsrepo = $self->opsrepo() || $self->conf()->getKey("core:PRIVATEOPSREPO");
