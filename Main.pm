...
#### GET LOGIN AND TOKEN FOR USER IF STORED IN DATABASE
	...
	$hubtype = $self->conf()->getKey("core:hubtype") if not defined $hubtype;
...
AND hubtype='$hubtype'};
	$self->logDebug("query", $query);
	my $keyfile = $self->table()->db()->query($query);
...
=cut
	...
	my $reposubdir = $self->conf()->getKey("core:REPOSUBDIR");
...
#AND package='$package'};
#    $self->logDebug("$query");
#    my $already_exists = $self->table()->db()->query($query);
#	my $now = $self->table()->db()->now();
#	
